...
class VideoSearchEngine:
    ...
    def process_video_old(self, 
                     ...
                     use_object_detection: bool = True) -> Dict[str, Any]:
        """
        Process a video file end-to-end (original version)
    ...
    def _search_single(self, query, **kwargs):
        """
        Perform a single search operation (original logic moved here)
        ...
        """
        # --- Original search logic from `search` method ---
        top_k = kwargs.get('top_k') or self.config.QUERY_TOP_K
        similarity_threshold = kwargs.get('similarity_threshold') or self.config.QUERY_SIMILARITY_THRESHOLD
        ...
        if kwargs.get('date_filter'):
            logger.info(f"Date filter: {kwargs.get('date_filter')}")
        if kwargs.get('namespace_filter'):
            logger.info(f"Namespace filter: {kwargs.get('namespace_filter')}")
        
        try:
            # Generate query embedding(s)
            query_embedding = self.embedding_generator.encode_query(query)
            # If dual embeddings enabled, also encode a CLIP text embedding for image index queries with fallbacks
            ...
            if kwargs.get('date_filter') and kwargs.get('namespace_filter'):
                # Search specific date + category namespace
                target_namespace = f"videos:{kwargs.get('date_filter')}:{kwargs.get('namespace_filter')}"
                ...
                date_namespaces = [ns for ns in namespaces.keys() if ns.startswith(f"videos:{kwargs.get('date_filter')}:")]
                
                if not date_namespaces:
                    logger.warning(f"No namespaces found for date: {kwargs.get('date_filter')}")
                    return []
            ...
            elif kwargs.get('namespace_filter'):
                # Search specific category across all dates
                # This requires querying multiple date namespaces
                stats = self.pinecone_manager.get_index_stats()
                namespaces = stats.get('namespaces', {})
                
                category_namespaces = [ns for ns in namespaces.keys() if ns.endswith(f":{kwargs.get('namespace_filter')}")]
                
                if not category_namespaces:
                    logger.warning(f"No namespaces found for category: {kwargs.get('namespace_filter')}")
                    return []
                ...
                if not namespaces:
                    ...
                    if getattr(self.config, 'ENABLE_DUAL_EMBEDDINGS', False) and clip_query_embedding is not None:
                        ...
                        search_results = self.pinecone_manager.semantic_search(
                            ...
                            video_filter=kwargs.get('video_filter'),
                    ...
                    if kwargs.get('video_filter'):
                        filtered_results = [r for r in filtered_results if r.video_name == kwargs.get('video_filter')]
                    
                    # Filter by time window if specified
                    if kwargs.get('time_window'):
                        start_time, end_time = kwargs.get('time_window')
    ...
    def search(self,
        ...
        try:
            ...
            # --- Run search for each expanded query and collect results ---
            all_results = []
            for q in expanded_queries:
                # Generate query embedding(s)
                query_embedding = self.embedding_generator.encode_query(q)
                ...
                if getattr(self.config, 'ENABLE_DUAL_EMBEDDINGS', False):
                    ...
                    for candidate in [getattr(self.config, 'CLIP_MODEL_NAME', None), 'clip-ViT-B-32', 'openai/clip-vit-base-patch32']:
                        ...
                        try:
                            from sentence_transformers import SentenceTransformer
                            clip_model = SentenceTransformer(candidate, device='cpu')
                            clip_query_embedding = clip_model.encode(q, convert_to_numpy=True, normalize_embeddings=True)
                            logger.info(f"Encoded query with CLIP model: {candidate}")
                            break
                        except Exception as e:
                            tried.append((candidate, str(e)))
                ...
                # The rest of the logic is the same as before, but for each expanded query
                # We'll collect all results and fuse them later
                ...
                if date_filter and namespace_filter:
                    ...
                    for ns in date_namespaces:
                        ...
                        all_ns_results.extend(ns_results)
                    all_ns_results.sort(key=lambda x: x.score, reverse=True)
                    search_results = all_ns_results[:top_k]
                    ...
                    if not namespaces:
                        ...
                        if getattr(self.config, 'ENABLE_DUAL_EMBEDDINGS', False) and clip_query_embedding is not None:
                            ...
                            for combined_id, fused_score in fused_items:
                                ...
                                if combined_conf < getattr(self.config, 'CONFIDENCE_THRESHOLD', 0.5):
                                    continue
                                final_results.append((r, fused_score * float(combined_conf)))
                            search_results = [r for r, score in final_results]
                        else:
                            search_results = self.pinecone_manager.semantic_search(
                                query_embedding=query_embedding,
                                top_k=top_k,
                        ...
                        for ns in namespaces.keys():
                            ...
                            all_ns_results.extend(ns_results)
                        filtered_results = [r for r in all_ns_results if r.score >= similarity_threshold]
                        if video_filter:
            ...
            # Flatten and fuse results from all expanded queries
            ...
            for results in all_results:
                for rank, item in enumerate(results):
                    key = item.frame_id if hasattr(item, "frame_id") else getattr(item, "id", None)
                    if not key:
                        continue
            ...
            fused_results = [result_map[k] for k in sorted_keys[:top_k]]
            ...
            for result in fused_results:
                formatted_result = {
                    ...
                    "frame_id": result.frame_id,
                    "video_name": result.video_name,
                    "video_date": result.metadata.get('video_date') if hasattr(result, "metadata") else None,
                    "cloudinary_url": result.metadata.get('cloudinary_url') if hasattr(result, "metadata") else None,
                    "time_formatted": self._format_timestamp(result.timestamp)
                }
                formatted_results.append(formatted_result)
